...
def salvar_parquet(df: pd.DataFrame, config: dict[str, Any]) -> Path:
    pasta_saida = Path(r"C:\Users\alexandresilva3\DaVita\Ciclo da Receita - HOSPITAL\Analise_De_Dados\Relatorios\0002 - Etapa Conta\Parquet")
    pasta_saida.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d-%H%M")
    nome_arquivo = f"EC-{timestamp}.parquet"
...
def main() -> None:
    ...
    caminho = salvar_parquet(df, config)
